...
extract_token_limit() {
  ...
  if command -v grep &> /dev/null; then
    echo "$ccusage_output" | grep -o "assuming [0-9,]* token limit" 2>/dev/null | grep -o "[0-9,]*" 2>/dev/null | tr -d ',' || true
  elif command -v powershell.exe &> /dev/null; then
    echo "$ccusage_output" | powershell.exe -Command '\$input | Select-String "assuming ([0-9,]*) token limit" | ForEach-Object { \$_.Matches[0].Groups[1].Value -replace ",","" }' 2>/dev/null || true
  else
    echo "$ccusage_output" | sed -n 's/.*assuming \([0-9,]*\) token limit.*/\1/p' 2>/dev/null | tr -d ',' || true
  fi
}
