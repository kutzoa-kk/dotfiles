...
送信前に実行する（**送信をブロックしない best-effort**）。**既定モデルは `GPT-5.6`**——ユーザーがモードを明示（例「Pro拡張で」「最高で」「標準で」）した場合はその指定を、無ければ `GPT-5.6` を `want` として選択する。現在モードが既に `want` なら選択操作はスキップされる（下の選択スニペットが早期 return）。既定モデルは選択スニペット冒頭の `DEFAULT_MODE` 1箇所で管理し、新モデルへ移行するときはそこだけ変更する。

**選択できなかったときの扱い（重要）**——`GPT-5.6` は本更新時点で実機ラベル未検証のため、必ず縮退経路を持たせ、モード選択を全送信の必須ゲートにしない。既定モデルでの縮退は**現在の状態のまま送信を続行**して止めないが、報告できる内容は失敗の種類で変わる（送信自体は composer/送信ボタンで行い、モードセレクタとは独立）:

- **既定 `GPT-5.6` がメニューに無い**（`mode not in menu`＝セレクタは見つかり現在モードも読めたが目的ラベルが無い。モデル選択が別 UI・ラベルドリフト等）→ セレクタから読めた**現在モード〈X〉のまま送信**し、`「GPT-5.6 を選択できず現在モード〈X〉で送信」`と明示。列挙スニペットで得た実在モードも提示し、次回の明示指定を促す。
- **モードセレクタ自体が無い**（`mode selector not found`＝ボタン未検出で**現在モード名の取得も列挙も不可**。UI 変更・現在ラベルが検出正規表現に不一致等）→ **送信は続行**するが現在モード名は特定できないので、`「モードセレクタを特定できず現在モード名不明のまま送信（UI 変更の可能性）」`とだけ明示する（モード名・列挙結果は取得不能ゆえ報告しない）。推測でセレクタを操作しない。
- **ユーザーが明示したモード**が見つからない（`mode not in menu` / `mode selector not found` いずれも）→ 要求と異なるモデルで送るのを避けるため **fail-loud で当該送信を中止**（その1件のみ。他の送信は妨げない）。

いずれも変更前の既定挙動と同じ安全側の縮退で、全送信が止まる事態を防ぐ（現在モードは実在の選択済みモデルであり推測ではない）。
...
選択後はセレクタボタンの innerText が `want` に変わったか確認。変わらなければ §0 冒頭の「選択できなかったときの扱い」に従う——**ユーザー明示モードなら fail-loud で当該送信を中止**、**既定 `GPT-5.6` なら中止せず送信を続行**し失敗の種類に応じて明示（未検証ラベルで全送信が止まるのを防ぐ）。

観測済みモード（2026-07-15 時点、ドリフトしうるので上の列挙で実行時確認）: `最速` / `標準` / `高` / `最高` / `Pro 拡張` / `GPT-5.6`（既定） / `Sol` / `Luna` / `Terra`。
- **GPT-5.6**（既定モデル。選択スニペットの `DEFAULT_MODE` で一元管理）: 旧 `GPT-5.5` は廃止。表示ラベルの正確な表記は実機の実行時列挙で確認する。選択できない場合（メニューに無い／モードセレクタ自体が無い／モデル選択が推論強度とは別 UI 等）は §0 冒頭の「選択できなかったときの扱い」に従う（既定モデルなので **fail-loud せず送信を続行**し、失敗の種類に応じて明示）。
