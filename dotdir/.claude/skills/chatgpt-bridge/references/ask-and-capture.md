...
**選択できなかったときの扱い（重要）**——`GPT-5.6` は本更新時点で実機ラベル未検証のため、必ず縮退経路を持たせ、モード選択を全送信の必須ゲートにしない:
- **既定 `GPT-5.6` を選択できない**（選択スニペットが `mode selector not found` / `mode not in menu` のいずれを返す場合も含む。ラベル未検証・モデル選択が別 UI・ラベルドリフト等）→ **スキルを止めない**。現在選択中のモードのまま送信を続行し、`「GPT-5.6 を選択できず現在モード〈X〉で送信」` と**明示**する。加えて列挙で得た実在モードを提示し、次回の明示指定を促す。これは変更前の既定挙動と同じ安全側の縮退で、全送信が止まる事態を防ぐ（現在モードは実在の選択済みモデルであり推測ではない）。
- **ユーザーが明示したモード**が見つからない → 要求と異なるモデルで送るのを避けるため **fail-loud で当該送信を中止**（その1件のみ。他の送信は妨げない）。
...
選択後はセレクタボタンの innerText が `want` に変わったか確認。変わらなければ §0 冒頭の「選択できなかったときの扱い」に従う——**ユーザー明示モードなら fail-loud で当該送信を中止**、**既定 `GPT-5.6` なら中止せず現在モードで続行＋明示**（未検証ラベルで全送信が止まるのを防ぐ）。

観測済みモード（2026-07-15 時点、ドリフトしうるので上の列挙で実行時確認）: `最速` / `標準` / `高` / `最高` / `Pro 拡張` / `GPT-5.6`（既定） / `Sol` / `Luna` / `Terra`。
- **GPT-5.6**（既定モデル。選択スニペットの `DEFAULT_MODE` で一元管理）: 旧 `GPT-5.5` は廃止。表示ラベルの正確な表記は実機の実行時列挙で確認する。メニューに無い場合（モデル選択が推論強度とは別 UI の可能性を含む）は §0 冒頭の「選択できなかったときの扱い」に従い、既定モデルなので **fail-loud せず現在モードで続行＋明示**する。
