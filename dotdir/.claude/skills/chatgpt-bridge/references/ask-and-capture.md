...
- **既定 `GPT-5.6` がメニューに無い**（ラベル未検証・モデル選択が別 UI・ラベルドリフト等）→ **スキルを止めない**。現在選択中のモードのまま送信を続行し、`「GPT-5.6 を選択できず現在モード〈X〉で送信」` と**明示**する。加えて列挙で得た実在モードを提示し、次回の明示指定を促す。これは変更前の既定挙動と同じ安全側の縮退で、全送信が止まる事態を防ぐ（現在モードは実在の選択済みモデルであり推測ではない）。
